...
Some cases the java.util.ConcurrentModificationException may occur, in that case re-run the simulator again as we have done anything to fine tune the code or to address the issue. The exception appears when multiple threads try to execute the same line of code. It doesn't happen always. Read the comments carefully before executing a piece of code.
Many places we have modified the existing the code to support our simulation such as DatacenterBroker.java
