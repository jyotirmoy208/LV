...
public class Datacenter extends SimEntity {
	...
	protected void processVmDestroy(SimEvent ev, boolean ack) {
		...
		//System.out.println("vm is getting distroyed is"+vm.getId()+"vm list size"+getVmList().size());
		getVmList().remove(vm);
	}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			}else if(estimatedFinishTime==0.0){
	 ...
	 */
	@SuppressWarnings("unchecked")
	public synchronized <T extends Vm> List<T> getVmList() {
