...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	@Override
public double getElasticUtilization(){
	double totalUtilization=0.0;
	double cloudLength=0.0;
	int numPes=0;

	synchronized(this) {
	for(ResCloudlet gl : getCloudletExecList()){
		
		cloudLength=gl.getCloudlet().getCloudletLength();
		numPes=gl.getCloudlet().getNumberOfPes();
		totalUtilization=totalUtilization+cloudLength*numPes;
		//System.out.println(" inside getElasticUtilization totalUtilization"+totalUtilization);
	}
	}
	return totalUtilization;
}
	...
	synchronized  public <T extends ResCloudlet> List<T>  getCloudletExecList() {
	...
	@SuppressWarnings("unchecked")
	@Override
	synchronized public <T extends ResCloudlet> List<T> getCloudletExecList4Elastic() {
		// TODO Auto-generated method stub
		return (List<T>) cloudletExecList;
	}
